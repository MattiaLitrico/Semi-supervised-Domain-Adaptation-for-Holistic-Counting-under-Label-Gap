...
def main():
    ...
    source_train = Cells(opt.source, mode='train')
    source_train_loader = DataLoader(source_train, batch_size=opt.batch, num_workers=opt.workers, shuffle=True)

    target_train = Cells(opt.target, mode='train', norm_label=True)
    target_test = Cells(opt.target, mode='validation', norm_label=True)
